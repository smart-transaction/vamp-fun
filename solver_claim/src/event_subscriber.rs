...
use crate::{cfg::Cfg, event_handler::ClaimHandler, events::ClaimToken};
...
impl SubscriberCallback {
    pub fn new(handler: Arc<ClaimHandler>) -> Self {
        Self { handler }
    }
}
...
impl Callback for SubscriberCallback {
    fn on_message(&self, message: &Message) -> std::result::Result<(), Box<dyn std::error::Error>> {
        let event: ClaimToken = message.unmarshal_to()?;
...
/// A RabbitMQ events subscriber. Receives events published by the event listener.
impl EventSubscriber {
    ...
    pub async fn listen(&mut self, deploy_token_handler: Arc<ClaimHandler>) -> Result<()> {
